<template>
  <div>
      ...
      <el-table stripe :data="data.tableData" @selection-change="handleSelectionChange">
        ...
        <el-table-column prop="userAvatar" label="用户头像">
          <template #default="scope">
            <el-image style="width: 50px; height: 50px; border-radius: 5px" :src="scope.row.userAvatar" :preview-src-list="[scope.row.userAvatar]" preview-teleported></el-image>
          </template>
        </el-table-column>
        <el-table-column prop="userName" label="用户名称"></el-table-column>
        <el-table-column prop="forumTitle" label="帖子">
          <template v-slot="scope">
            <a style="color: blue" :href="'/manager/forumDetail?id=' + scope.row.fid">{{ scope.row.forumTitle }}</a>
          </template>
        </el-table-column>
    ...
    <el-dialog title="评论信息" v-model="data.formVisible" width="40%" destroy-on-close>
      <el-form ref="form" :model="data.form" label-width="70px" style="padding: 20px">
        <el-form-item prop="title" label="评论内容">
          <el-input v-model="data.form.content" placeholder="请输入评论内容"></el-input>
        </el-form-item>
      </el-form>
      <template #footer>
